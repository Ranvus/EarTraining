...
signal pianoUnisonIntervalScoreChanged
...
func _process(_delta):
	Global.piano4_score = piano4_score
	...
	if can_play and note_group.size() == 0:
		...
		for i in range(2):
			if i == 0:
				...
				if notes.find(note_array[i-1])+4 < notes.size():
					rand_note = notes[randi_range(notes.find(note_array[i-1]), notes.find(note_array[i-1])+4)]
				else:
					rand_note = notes[randi_range(notes.find(note_array[i-1]), notes.find(note_array[i-1])-4)]
...
func right_answer():
	if interval_answer == player_node.answer:
		piano4_score += 1
		pianoUnisonIntervalScoreChanged.emit()
		notes_destroy()
	else:
		player_node.penalty()
		notes_destroy()
